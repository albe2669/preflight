...
mod common;
use common::*;
use entity::sea_orm_active_enums::{EventKind, TodoStatus};
use pretty_assertions::assert_eq;
use sea_orm::{ColumnTrait, EntityTrait, QueryFilter};

use preflight_core::{Error, todo_service::TodoService};

#[tokio::test]
async fn test_create_todo_returns_model_with_defaults() {
    let db = setup_db().await;
    let service = preflight_core::todo_service::new(db, test_clock());
...
async fn test_create_todo_emits_created_event() {
    let db = setup_db().await;
    let service = preflight_core::todo_service::new(db.clone(), test_clock());

    let todo = service.create("Buy milk".to_string(), None).await.unwrap();

    let events = entity::todo_event::Entity::find()
        .filter(entity::todo_event::Column::TodoId.eq(todo.id))
        .all(&db)
        .await
        .unwrap();
...
async fn test_update_changes_title_and_emits_event() {
    ...
    let events = entity::todo_event::Entity::find()
        .filter(entity::todo_event::Column::TodoId.eq(todo.id))
        .filter(entity::todo_event::Column::Kind.eq(EventKind::TitleChanged))
        .all(&db)
        .await
        .unwrap();
    assert_eq!(events.len(), 1);
