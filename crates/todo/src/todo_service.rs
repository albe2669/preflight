...
use crate::entity::enums::{EventActor, EventKind, TodoStatus};
use crate::entity::todo::{ActiveModel, Entity, Model};
use sea_orm::{
...
impl TodoService for TodoServiceImpl {
    ...
    async fn all(&self) -> Result<Vec<Model>> {
        Ok(Entity::find()
            .order_by_desc(crate::entity::todo::Column::CreatedAt)
            .all(&self.db)
            .await?)
    }
    ...
    async fn find_by_title(&self, title: &str) -> Result<Option<Model>> {
        Ok(Entity::find()
            .filter(crate::entity::todo::Column::Title.eq(title))
            .one(&self.db)
            .await?)
    }
}
