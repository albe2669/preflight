//! Hand-maintained ActiveEnums for the todo domain.
//!
...
//!
//! `EventKind` intentionally has **no** SQL `CHECK`: it mirrors a domain that
//! grows without asking. The enum is the guard on read; unknown values surface
//! as a SeaORM error. Add a variant when you adopt a new value, never a SQL
//! migration.
//!
...
//! enum args and return types. These impls must live in the defining crate
//! (orphan rule).
