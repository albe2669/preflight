//! `sync_state` upsert helpers for the github source.
//!
//! This is a thin adapter over the shared [`remote_sync::cursor`], mapping
//! its [`CursorError`] onto [`GithubError::Db`] so callers keep returning
...
fn map_err(e: CursorError) -> GithubError {
    GithubError::Db(e.into())
}
